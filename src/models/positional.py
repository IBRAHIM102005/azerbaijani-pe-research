...
class SinusoidalPositionalEncoding(PositionalScheme):
    ...
    def additive_embedding(
        ...
        if seq_len > self.table.shape[0]:
            # Longer than the precomputed table: recompute for this call only.
            # The formula is closed-form, so this is exact -- but the buffer is
            # deliberately NOT replaced, because mutating a registered buffer
            # inside forward would make the module's state depend on the order
            # in which batches happened to arrive.
            table = sinusoidal_table(
                seq_len, self.d_model, self.theta, device=device, dtype=dtype
            )
