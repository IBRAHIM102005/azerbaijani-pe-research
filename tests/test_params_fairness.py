...
#: any preregistered seed works for structural tests; 17 is the first
TEST_SEED = 17
...
def production_config(pe_type: str = "nope", seed: int = TEST_SEED) -> ModelConfig:
    """The real base config, loaded from disk, with a run seed resolved.

    The shipped file carries the template seed, so a seed must be stamped
    before a model can be built -- that is the point of the sentinel.
    """
    path = Path(__file__).resolve().parents[1] / "configs" / "model_base.json"
    return ModelConfig.from_json(path).with_pe(pe_type).with_seed(seed)
