...
def test_factory_builds_every_arm(pe_type):
    cfg = ModelConfig(pe_type=pe_type, init_seed=17, vocab_size=64, d_model=32, n_head=4,
...
def test_length_generalisation_contract(eval_len):
    ...
    for pe_type in PE_TYPES:
        cfg = ModelConfig(pe_type=pe_type, init_seed=17, max_seq_len=trained_len)
        pe = build_positional_scheme(cfg)
...
def test_relative_arms_extrapolate_without_new_parameters():
    for pe_type in ("sinusoidal", "rope", "alibi", "nope"):
        cfg = ModelConfig(pe_type=pe_type, init_seed=17, max_seq_len=512)
        pe = build_positional_scheme(cfg)
...
def test_spec_rotary_pct_applied_to_the_real_geometry():
    ...
    path = Path(__file__).resolve().parents[1] / "configs" / "pe" / "rope.json"
    cfg = ModelConfig.from_json(path).with_seed(17)
...
def test_sinusoidal_long_call_does_not_mutate_the_registered_buffer():
    """Module state must not depend on the order batches happened to arrive."""
    pe = SinusoidalPositionalEncoding(16, 32)
    before = pe.table.clone()
    out = pe.additive_embedding(40, torch.device("cpu"), torch.float32)
    assert out.shape == (1, 40, 32)
    assert pe.table.shape == before.shape
    assert torch.equal(pe.table, before)
    # and the long result still agrees with the reference formula
    reference = sinusoidal_table(40, 32)
    assert torch.allclose(out[0], reference, atol=1e-6)
